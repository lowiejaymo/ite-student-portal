<?php
/*
officer-add-event-be.php
Authors:
  - Lowie Jay Orillo (lowie.jaymier@gmail.com)
  - Caryl Mae Subaldo (subaldomae29@gmail.com)
  - Brian Angelo Bognot (c09651052069@gmail.com)
Last Modified: June 12, 2024
Overview: This file handles the editing events, validating officer input and inserting the event into the database.
*/
...
if (isset($_POST['addEvent'])) {
    ...
    if (empty($event_name)) {
        header("Location: ../officer-event-edit.php?newEventError=Event name is required&$user_data");
        exit();
    } // Validate date if empty
    elseif (empty($date)) {
        header("Location: ../officer-event-edit.php?newEventError=Date is required&$user_data");
        exit();
    } // Validate school year if empty
    elseif (empty($schoolyear)) {
        header("Location: ../officer-event-edit.php?newEventError=School year is required&$user_data");
        exit();
    } // Validate semester if empty
    elseif (empty($semester)) {
        header("Location: ../officer-event-edit.php?newEventError=Semester is required&$user_data");
        exit();
    } elseif (empty($points)) {
        header("Location: ../officer-event-edit.php?newEventError=Points is required&$user_data");
        exit();
    } else {
        ...
        if (mysqli_num_rows($result_check_existing) > 0) {
            header("Location: ../officer-event-edit.php?newEventError=Event name already exists&$user_data");
            exit();
        } else {
            // Update event
            ...
            if ($result_update_event) {
                header("Location: ../officer-event-view.php?&event_id=$event_id&updateEventSuccess=Event updated successfully");
                exit();
            } else {
                header("Location: ../officer-event-edit.php?updateEventError=Failed to update event&$user_data");
                exit();
            }
        }
    }
