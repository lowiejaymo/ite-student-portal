<?php
/*
officer-students-reset-password.php
Authors:
...
if (isset($_POST['account_number'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if ($update_stmt->execute()) {
            header("Location: ../officer-student-view.php?account_number=$account_number&resetSuccess=Password reset successfully");
        } else {
            header("Location: ../officer-student-view.php?account_number=$account_number&resetError=Failed to reset password");
        }
    } else {
        header("Location: ../officer-student-view.php?account_number=$account_number&resetError=Student not found");
    }
} else {
