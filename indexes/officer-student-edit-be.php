...
if (isset($_POST['editStudent'])) {
    ...
    if (empty($accountnumber)) {
        header("Location: ../officer-student-edit.php?editStudentError=Account Number is required&$user_data");
        exit();
    } // Validate last name if empty
    elseif (empty($lastname)) {
        header("Location: ../officer-student-edit.php?editStudentError=Last Name is required&$user_data");
        exit();
    } // Validate first name if empty
    elseif (empty($firstname)) {
        header("Location: ../officer-student-edit.php?editStudentError=First Name is required&$user_data");
        exit();
    } // Validate program if empty
    elseif (empty($program)) {
        header("Location: ../officer-student-edit.php?editStudentError=Program is required&$user_data");
        exit();
    } // Validate year level if empty
    elseif (empty($yearlevel)) {
        header("Location: ../officer-student-edit.php?editStudentError=Year level is required&$user_data");
        exit();
    } // Validate gender if empty
    elseif (empty($gender)) {
        header("Location: ../officer-student-edit.php?editStudentError=Gender is required&$user_data");
        exit();
    } else {
            ...
            if ($result_updatestudent_query) {
                header("Location: ../officer-students.php?editStudentSuccess=Student updated successfully");
                exit();
            } else {
                header("Location: ../officer-student-edit.php?editStudentError=Failed to update student account&$user_data");
                exit();
            }
    }
} else {
