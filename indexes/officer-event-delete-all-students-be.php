...
if (isset($_POST['add_all'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($insertSuccess) {
                header("Location: ../officer-event-view.php?event_id=$event_id&addAllSuccess=All eligible students have been delete to the event.");
            } else {
                header("Location: ../officer-event-view.php?event_id=$event_id&addAllError=Failed to delete all eligible students.");
            }
        } else {
            header("Location: ../officer-event-view.php?event_id=$event_id&addAllError=No students found to delete to the event.");
        }
    } else {
