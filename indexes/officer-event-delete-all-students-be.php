...
if (isset($_POST['add_all'])) {
    ...
    if (!$stmt) {
        header("Location: ../officer-event-view.php?addAllError=SQL Error");
        exit();
    }
    ...
    if ($result->num_rows > 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($deleteSuccess) {
                header("Location: ../officer-event-view.php?event_id=$event_id&addAllSuccess=All eligible students have been deleted from the event.");
            } else {
                header("Location: ../officer-event-view.php?event_id=$event_id&addAllError=Failed to delete all eligible students.");
            }
        } else {
            header("Location: ../officer-event-view.php?event_id=$event_id&addAllError=No students found to delete from the event.");
        }
    } else {
